...
feature
	...
	do
		if x < 1 or x > 8 or y < 1 or y > 8 then
			...
			if color = True then -- White bishop
				if abs (y, position_y) = abs (x, position_x) and (chess_field.table.item(x, y) /= 1) then
					io.put_string ("You successfully moved white bishop from " + position_x.out + "," + position_y.out + " to " + x.out + "," + y.out + "!%N%N")
				...
				if abs (y, position_y) = abs (x, position_x) and (chess_field.table.item(x, y) /= 2) then
					io.put_string ("You successfully moved black bishop from " + position_x.out + "," + position_y.out + " to " + x.out + "," + y.out + "!%N%N")
					chess_field.table.put (0, position_x, position_y)
					position_x := x
					position_y := y
